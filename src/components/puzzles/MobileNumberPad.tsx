...
const MobileNumberPad = ({ onNumber, onDelete, visible, maxDigit = 9 }: Props) => {
  ...
  return (
    <div className="grid grid-cols-5 gap-1.5 mt-3 max-w-[280px]">
      {Array.from({ length: maxDigit }, (_, i) => (
        <button
          key={i + 1}
          type="button"
          className={cn(
            "min-h-[44px] rounded-lg border border-border bg-card text-foreground font-semibold text-lg",
      ...
      <button
        type="button"
        className={cn(
          "min-h-[44px] rounded-lg border border-border bg-card text-muted-foreground font-semibold text-sm",
